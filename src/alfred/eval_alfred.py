...
def evaluate(env, traj_data, r_idx, model_args, planner, save_path):
    # setup scene
    ...
    # save
    save_result(log_entry, imgs, save_path)

    return log_entry


def save_result(result_dict, imgs, base_path='results'):
    if result_dict:
...
def run(tasks, args_dict, planner, x_display, save_path):
    results = []
    ...
    for task in tqdm(tasks):
        try:
            ...
            result = evaluate(env, traj_data, r_idx, model_args, planner, save_path)
...
def main(cfg: DictConfig):
    ...
    results = run(files, args_dict, planner, x_display, save_path)
